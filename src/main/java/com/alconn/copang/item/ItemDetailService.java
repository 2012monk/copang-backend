...
//        }
        ...
        List<ItemDetail> itemDetailList = itemMapper
            .listDtoToDomainN(itemForm.getItemDetailFormList());
...
//    public ItemForm updateItemDetail(ItemForm.ItemFormUpdate itemFormUpdate){
    public ItemForm.ItemFormUpdate updateItemDetail(ItemForm.ItemFormUpdate itemFormUpdate)
        throws NoSuchEntityExceptions {
        //풀기
        List<ItemDetailForm.DetailUpdateClass> iDetailUpdateClasses = itemFormUpdate
            .getItemDetailUpdateClassList();

        //item이 바뀌었는지 확인
        Item item = itemService.itemFindNum(itemFormUpdate.getItemId());

        boolean perceive = false;
        if (!item.getItemName().equals(itemFormUpdate.getItemName()) || !item.getItemComment()
            .equals(itemFormUpdate.getItemComment())) {
            //=====
        ...
        List<ItemDetail> itemDetailList = itemDetailRepository
            .findItemDetailPage(itemFormUpdate.getItemId());
...
//        }
//=====
        ItemDetail itemDetail = itemDetailRepository
            .findById(updateSingle.getDetailUpdateClass().getItemDetailId())
